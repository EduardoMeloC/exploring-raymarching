...
Thanks to the use of outer bounds for the clouds, the optimized Sphere Tracing (Figure \ref{fig:spheretracing}) is kept as normal, up until a cloud bound is hit, at line 13. At this point, Fixed-Step Ray Marching begins, and either stops when a hard surface is hit (line 18) or when the vision is fully clouded (line 22). As discussed, the \texttt{GetDensity(p)} function acts as the \texttt{GetDist(p)} for cloudy objects, so for a cloudy sphere, it may simply look like Code 8, and produce the effect in Figure \ref{fig:no-noise-cloud}. It certainly looks cloudy, but still lacks the organic shapes of a conventional cloud.
