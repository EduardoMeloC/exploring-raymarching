...
Firstly, to render gaseous materials, instead of avoiding going into surfaces, Ray Marching really needs to traverse them. For that, the SDF now represents density instead of "distance to surface", and for the sake of clarity, they are inversed so that the distance to a point $\mathbf{p}$ follows certain rules. This function is going to be called $\mathbf{GetDensity(p)}$, and the rules are:

\begin{itemize}
    \item If p is outside of the primitive, yields a negative density, or no density.
    \item If p is inside of the primitive, yields a positive density, and the further p is from bounds, the higher density is.
\end{itemize}

Secondly, to consistently sample the density along the cloud, a constant step size is better suited - instead of querying the scene's distance for the next marching step, each step will perform a constant size. Of course, this comes at the expense of performance, losing the optimizations discussed in section 2.3.
...
The pseudocode in algorithm \ref{alg:marchray_cloud} builds upon algorithm \ref{alg:marchray} to describe the relationship between clouds and regular surfaces. It's a lenghy, yet simple procedure in theory. The first difference comes with the addition of $\mathbf{accColor}$, a 4D vector describing the RGBA channels. Red, blue and green fields store the accumulated color of the pixel and the alpha channel stores the accumulated density. This makes the variable $\mathbf{isHit}$ deprecated. Since the $\mathbf{accColor.a}$ channel ranges from 0 to 1, $\mathbf{accColor.a = 1}$ means the ray path is now fully clouded, that being from a straight hard surface, or the accumulation of density between cloudy masses. Said check happens in lines 22 and 26.

Thanks to the use of outer bounds for the clouds, the optimized Sphere Tracing (Figure \ref{fig:spheretracing}) is kept as normal, up until a cloud bound is hit, at line 13. At this point, Fixed-Step Ray Marching begins, and either stops when a hard surface is hit (line 18) or when the vision is fully clouded (line 22). As discussed, the $\mathbf{GetDensity(p)}$ function acts as the SDF for cloudy objects, so for a cloudy sphere, it may simply look like Code 8, and produce the effect in Figure \ref{fig:no-noise-cloud}. It certainly looks cloudy, but still lacks the organic shapes of a conventional cloud.

\begin{lstlisting}[language=GLSL, caption={Code 8: Get density of spherical cloud}, label={lst:GetDensity} float=H]
float GetDensity(p){
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \small vn(uv)+vn(uv*2.)*0.5+vn(uv*4.)*0.25
    \end{minipage}%
    \hfill
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \small vn(uv)+vn(uv*2.)*0.5+vn(uv*4.)*0.25+vn(uv*8.)*0.125
    \end{minipage}%
    \hfill
...
\begin{lstlisting}[language=GLSL, caption={Code 9: Lighting the Cloud Volume}, label={lst:Displacement} float=H]
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=9.5cm]{imagens/cloud-noise/lit-cloud.png}}
