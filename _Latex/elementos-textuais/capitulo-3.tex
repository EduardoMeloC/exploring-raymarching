\chapter{Signed Distance Functions Techniques}\label{chp:LABEL_CHP_3}
...
\begin{equation}\label{eq:circleSDF}
...
\begin{lstlisting}[language=GLSL, caption={Code 3: Rendering Circles}, label={lst:ColoringCircle} float=ht]
...
Since lighting models are outside the scope of this work, readers may refer to \cite{akenine-moller2018realtimerendering} for further details on shading techniques.
...
Similar to the 2D circle, the signed distance function (SDF) of a 2D box, or rectangle, can be generalized to higher dimensions (see Code 4).

\begin{lstlisting}[language=GLSL, caption={Code 4: Rectangle and Box SDF}, label={lst:RectangleAndBox} float=ht]
...
The rectangle SDF is defined at the origin $(0,0)$, and due to the rectangle's symmetry, it is often more convenient to work within the first cartesian quadrant. To achieve this, $\mathbf{b}$ (half each box`s side) is subtracted from $\texttt{abs}(\mathbf{p})$, yielding a point $\mathbf{d}$. As illustrated in Figure \ref{fig:square_sdf}:
...
It can be seen that when both $\mathbf{d}.x$ and $\mathbf{d}.y$ are limited to zero, which visually means snapping all points to Section 2, the distance to the corner encases the results, although not entirely. If in Section 4, inside the rectangle, $\texttt{length}(\texttt{max}(\mathbf{d},0.0))$ is always 0. In such cases, the SDF should return the negative distance to the nearest edge of the rectangle, adding the expression $\texttt{min}(\texttt{max}(\mathbf{d}.x,\mathbf{d}.y),0.0)$. The \texttt{min()} function ensures non-zero distances are only possible in Section 4. With that, all sections are considered.
...
All primitives described in the previous Section were centered at the origin. In order to perform a transformation such as scaling, rotation or translation, it is possible to apply the inverse transformation at the query point $\mathbf{p}$: this way, we are transforming the space, and not the shape directly.

In Code 5, by subtracting the vector $(0., 3., 0.)$, the sphere is effectively positioned at $(0., 3., 0.)$.

\begin{lstlisting}[language=GLSL, caption={Code 5: Translating Sphere}, label={lst:TransformingSphere} float=ht]
float transformedSphere(vec3 point)
{
    return sphereDistance(point - vec3(0., 3., 0.));
}
\end{lstlisting}

Affine transformations can also be handled directly using matrices. GLSL provides built-in support for creating and inverting matrices. Code 6 shows how different affine transformations can be composed, and the result is illustrated in Figure \ref{fig:affineTransformations}.

\begin{lstlisting}[language=GLSL, caption={Code 6: Affine Transformation on Sphere}, label={lst:AffineTransformationSphere} float=ht]
float transformedSphere(vec3 point)
{
    // Scale 2X along Y
    mat4 S = mat4(
    ...
    mat4 transformationMatrix = S * R * T;
    
    vec3 transformedPoint = (vec4(point, 1.) * inverse(transformationMatrix)).xyz;

    return sphereDistance(transformedPoint);
}
\end{lstlisting}
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imagens/transformations/1-transformations-identity.png}\\
        Original Sphere
    \end{minipage}%
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imagens/transformations/2-transformations-scale.png}\\
        Scale 2X along Y
    \end{minipage}
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imagens/transformations/3-transformations-rotation.png}\\
        Rotation in XY
    \end{minipage}%
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imagens/transformations/4-transformations-translation.png}\\
        Translate to (-2., -1., -1.)
    \end{minipage}

    \caption{Composing Different Affine Transformations on a Sphere}
    \label{fig:affineTransformations}
\end{figure}
...
Operations between objects are some of the most elegant features of SDFs, and the most basic ones are the combinations: Union, Intersection and Subtraction. These three have very simple definitions and are great ways to introduce SDF operations. They take as input the SDF resulting distance of two primitive SDFs $\mathbf{d_1}$ and $\mathbf{d_2}$, as described in Code 7.


\begin{lstlisting}[language=GLSL, caption={Code 7: SDF Union, Intersection and Subtraction}, label={lst:UnionIntersectionSubtraction} float=ht]
...
From the perspective of a 3D raymarched vision, given $\mathbf{d_1}$ and $\mathbf{d_2}$, SDF distances from objects 1 and 2:
From the perspective of a 3D raymarched vision, given $\mathbf{d_1}$ and $\mathbf{d_2}$, SDF distances from objects 1 and 2:

\begin{itemize}
    \item Taking the minimum between $\mathbf{d_1}$ and $\mathbf{d_2}$ is like seeing only the closest surface from both objects to camera, which is already the case for solids seen from the outside.
    \item Taking the maximum between $\mathbf{d_1}$ and $\mathbf{d_2}$ means that nothing outside the intersection of both objects is seen, since the ray march will only consider a hit if both SDF results are below the hit threshold.
    \item Taking the maximum between $\mathbf{-d_1}$ and $\mathbf{d_2}$ is like intersecting with the inverse of $\mathbf{d_1}$, which means that all $\mathbb{R}^3 $ except object1. This results in the subtraction of objects 1 from 2.
\end{itemize}

Figure \ref{fig:sdf_operations} illustrates these constructive solid geometry operations between a cube and a sphere.
...
This blending can be achieved through multiple types of functions, such as quadratic, cubic, exponential, sigmoid, each having it`s pros and cons \cite{quilez2013smin}.
...
Because it is fast and does not overestimate distances, the quadratic polynomial is less prone to visual artifacts and is the most widely used version for Ray Marching, thus being the one this Section focuses on. Similarly to the regular \texttt{min} function, \texttt{smin} (Smooth Minimum) takes as input $\mathbf{a}$ and $\mathbf{b}$, which could be distances, or points in space, plus the threshold $k$. The blending starts where the distance between $\mathbf{a}$ and $\mathbf{b}$ is smaller than $k$.

Figure \ref{fig:smin} will help navigate the explanation ahead, $a\ =\ -\left(2-x\right)^{2}+2$ is in red and $b\ =\ \left(2-x\right)^{2}\ +.5$ in blue. To find \texttt{smin}, all polynomial methods start by finding the $h()$ function, which is 0 by default, and ramps up to 1, starting at $|\mathbf{a} - \mathbf{b}|=k$ and peaking at $|\mathbf{a} - \mathbf{b}|=0$:

\begin{equation}
    \mathbf{h} = \frac{\max(\mathbf{k} - \|\mathbf{a} - \mathbf{b}\|, 0.0)}{\mathbf{k}}
    \label{eq:example_equation}
\end{equation}

Since $k$ is the distance in which the shapes start to combine, $h()$ helps delimitate exactly that, as represented in green, Figure \ref{fig:smin}. Next, there needs to be a $c$ such that $\texttt{min}{(\textbf{a},\textbf{b})-c}$ produces the smoothed curve. This is no random requirement, but a simplified structure of what is the DD (Direct Difference) family of \texttt{smin} solutions. For polynomials, a simple solution for $c$ is as follows, with $n>1.5$ already producing great results:

\begin{equation}
    \mathbf{c}\ =\frac{\mathbf{h}^{\mathbf{n}}\mathbf{k}}{\mathbf{2n}}
    \label{eq:example_equation_2}
\end{equation}

Between all values of $\mathbf{n}$, the quadratic version ($\mathbf{n=2}$) produces the gentler of curves, and is prioritized over the others. To better visualize what $c$ means, Figure \ref{fig:sunion} represents $c$ in purple. Squaring ${h}$ makes $c$ ease into 1, instead of following a straight ramp. Multiplying by $k$ compensates any variation, as the peak of ${h}$ decreases as $k$ increases. Lastly, the divisor helps position the peak to the right place.
To achieve smooth maximum, all that is needed is $\texttt{max}(\mathbf{a},\mathbf{b})+c$.

A visualization of the addition, subtraction, and intersection operations, along with their smooth counterparts, is shown in Figure \ref{fig:smooth_sdf_operations}.
...
This can be achieved through the natural repetitive pattern of trigonometric functions such as $sin(x)$ and $cos(x)$, however, bending the domain with these functions will compromise the accurate Euclidean distances of the space the SDFs relies on, and this might generate features that are not desired. Functions such as $round(x)$ or $mod(x)$ (modulus operator) are more commonly used instead.
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \footnotesize\texttt{\mbox{circleDistance(uv, 0.1)}}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imagens/domainRepetition-sine.png}\\
        \footnotesize\texttt{\mbox{circleDistance(sin(uv * 5.)*0.2, 0.1)}}
    \end{minipage}
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \footnotesize\texttt{circleDistance(uv - round(uv), 0.1)}
    \end{minipage}%
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imagens/domainRepetition-mod.png}\\
        \footnotesize\texttt{circleDistance(mod(uv, 1.)-0.5, 0.1)}
    \end{minipage}
...
A key property of 2D and 3D Signed Distance Functions is that their gradient has length $1$ everywhere in the space. Since SDFs measure distances, the rate of change of distance must be equal to $1$. When two SDFs are added, this property is violated: the gradient no longer has unit length, effectively ``bending'' space. This can cause the raymarcher to underestimate or overestimate the true distance from the surface.
...
\begin{lstlisting}[language=GLSL, caption={Code 9: Wave Distortion on Sphere}, label={lst:DistortionOnSphere} float=ht]
...
\begin{figure}[ht]
  ...
  \captionof{figure}{Distortion on Sphere from Code 9}
  \label{fig:sin-cos-distortion}
\end{figure}

In Code 9, the Sphere Distance is distorted according to the trigonometric functions applied on its polar and azimutal angle. The result is shown in Figure \ref{fig:sin-cos-distortion}.