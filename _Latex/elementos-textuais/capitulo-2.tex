...
The pseudocode explicitly uses nested loops to traverse the canvas. In the GLSL implementation that follows, the iteration logic is unnecessary because the fragment shader operates on a per-pixel basis. Each invocation of the shader corresponds to a single pixel on the canvas, and the GPU automatically handles the parallel execution of the shader for all pixels. The input \texttt{fragCoord} provides the current pixel's coordinates, \texttt{vec2} and \texttt{vec3} represent 2D and 3D vectors respectively, and GLSL functions handle vector operations like normalization and dot products. 
...
In the presented example, the scene is simplified to a single sphere. Since the analytic solution for ray-sphere intersection is well known \cite{turn0search0}, the surface can
be rendered smoothly and accurately without relying on geometric tessellation.

The \texttt{GetHitColor} function currently returns only the object's albedo. To simulate realistic lighting behavior, additional calculations are required.
...
To illustrate a limitation of the method, Figure \ref{fig:unlit_march_sphere} was rendered with a low step cap of 150. Careful inspection reveals that the distant ground plane disappears near the edges of the sphere. This occurs because rays take smaller steps when approaching a surface, requiring more iterations to traverse its boundary. As a result, the step budget is exhausted before the rays can reach the the ground plane. Fortunately, 150 steps is well below the typical \texttt{MAX\_MARCH\_STEPS}. Using a higher value removes this artifact with little added cost, and beyond this edge case, most rays terminate within fewer than 10 steps due to the scene's distance limits.
